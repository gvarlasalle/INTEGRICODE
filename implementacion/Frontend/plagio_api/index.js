...
app.post('/extract-code', async (req, res) => {
    ...
    if (!url) {
        return res.status(400).json({ error: 'url no valido' });
    }
    ...
    try {
        ...
        if (!response || response.status() === 404) {
            await browser.close();
            return res.status(200).json({ error: 'URL no válida' });
        }
        ...
        res.status(500).json({ error: 'Url no valida', details: error.message });
    }
});
...
app.get('/api/dni', async (req, res) => {
    ...
    const dniPattern = /^\d{8}$/; // Expresión regular para 8 dígitos numéricos
